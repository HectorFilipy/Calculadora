    ...
    def _config_buttons(self):
        ...
        for row_values in buttons:
            for button in row_values:
                ...
                if button_text == '=':
                    button.bind('<Button-1>', self.resultado)
                    button.config(bg='#4785F4', fg='#fff')
    
    def _config_display(self):
        self.display.bind('<Return>', self.resultado)
        self.display.bind('<KP_Enter>', self.resultado)

    def _fix_text(self, text):
        # Substitui tudo que não for 0123456789./*-+^() para nada
        text = re.sub(r'[^\d\.\/\*\-\+\^\(\)e]', r'', text, 0)
        # Substitui operadores repeditos para apenas um operador
